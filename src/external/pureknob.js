  ...
  this.createBarGraph = function (width, height) {
    ...
    const graph = {
      ...
      setValue: function (value) {
         ...
         */
        if (value < valMin) { value = valMin } else if (value > valMax) { value = valMax }
  ...
  this.createKnob = function (width, height) {
    ...
    const knob = {
      ...
      redraw: function () {
         ...
         */
        if (needle) { ctx.arc(centerX, centerY, radius, angleVal - 0.1, angleVal + 0.1) } else { ctx.arc(centerX, centerY, radius, actualStart, angleVal) }
    ...
    const mouseEventToValue = function (e, properties) {
       ...
       */
      if (angle < 0) {
        if (angleDiff >= twoPi) { angle += twoPi } else { angle = 0 }
      }
...
const pureknobInstance = pureknob.createKnob(5, 5)

export default pureknob
export { pureknobInstance }
